...
class BalancedGroupKFold(GroupKFold):
    ...
    def __init_data(self, y, groups):
        ...
        for unique_class, samples in zip(unique_classes, sample_counts):
            ...
            unique_groups, samples_in_group = np.unique(groups[indices], return_counts=True)
            ...
            multiplier = np.ceil(sample_counts[smallest_class_index] * self.n_splits / samples).astype(np.uint8)
            ...
            self.__class_information[unique_class] = {
                ...
                "start_index": 0,
                "extra_samples": np.argmin(np.abs(
                    np.cumsum(samples_in_group[shuffle_indices]) - self.n_splits * sample_counts[smallest_class_index]
                )),
            }
    ...
    def split(self, X, y=None, groups=None):
        ...
        for train_idx, test_idx in self.__cv_smallest_class.split(
            ...
            train_groups = np.unique(self.__class_information[self.__smallest_class]["sample_group"][train_idx])
            test_groups = np.unique(self.__class_information[self.__smallest_class]["sample_group"][test_idx])

            # map train and test indices to original indices
            train_idx = self.__class_information[self.__smallest_class]["sample_indices"][train_idx]
            test_idx = self.__class_information[self.__smallest_class]["sample_indices"][test_idx]
            ...
            assert len(train_idx) == len(np.unique(train_idx)), "Duplicate samples used in train set"
            assert len(test_idx) == len(np.unique(test_idx)), "Duplicate samples used in test set"

            assert self.__count_use["train"].max() <= self.n_splits, "Train sample used more than n_splits times"
            assert self.__count_use["test"].max() <= self.n_splits, "Test sample used more than n_splits times"
    ...
    @staticmethod
    def __resolve_group_overlap(class_information, current_set_groups, number_of_samples, other_sets_groups,
                                result_idx, sample_usage):
        cum_sum = np.cumsum(class_information["samples_in_group"][class_information["start_index"]:])
        ...
        remaining_groups = class_information["groups"][class_information["start_index"]:]
        remaining_samples_in_group = class_information["samples_in_group"][class_information["start_index"]:]
        ...
        if common_groups.size > 0:
            ...
            for group_to_replace_idx, group_to_replace in zip(common_group_idx, common_groups):
                new_group_idx = -1
                ...
                while (new_group in other_sets_groups or
                       new_group in current_groups or
                       class_information["samples_in_group"][new_group_idx] != group_to_replace_samples or
                       current_sample_usage != min_use):
                    new_group_idx -= 1
                ...
                current_groups[group_to_replace_idx] = class_information["groups"][new_group_idx]

        idx = class_information["sample_indices"][np.isin(class_information["sample_group"], current_groups)]
        class_information["start_index"] += i + 1
        return np.concatenate([result_idx, idx]), np.concatenate([current_set_groups, current_groups])
...
if __name__ == "__main__":
    ...
    metadata = pd.read_csv(DATASETS["MIAS"].METADATA.FILE, **DATASETS["MIAS"].METADATA.PARAMETERS)
