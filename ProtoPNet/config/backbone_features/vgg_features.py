...
def vgg11_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        my_dict = model_zoo.load_url(model_urls["vgg11"], model_dir=PRETRAINED_MODELS_DIR)
        keys_to_remove = set()
        for key in my_dict:
...
def vgg11_bn_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        my_dict = model_zoo.load_url(
            model_urls["vgg11_bn"], model_dir=PRETRAINED_MODELS_DIR
        )
        keys_to_remove = set()
        for key in my_dict:
            if key.startswith("classifier"):
...
def vgg13_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        my_dict = model_zoo.load_url(model_urls["vgg13"], model_dir=PRETRAINED_MODELS_DIR)
        keys_to_remove = set()
        for key in my_dict:
            if key.startswith("classifier"):
                keys_to_remove.add(key)
...
def vgg13_bn_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        my_dict = model_zoo.load_url(
            model_urls["vgg13_bn"], model_dir=PRETRAINED_MODELS_DIR
        )
        keys_to_remove = set()
        for key in my_dict:
            if key.startswith("classifier"):
...
def vgg16_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        my_dict = model_zoo.load_url(model_urls["vgg16"], model_dir=PRETRAINED_MODELS_DIR)
        keys_to_remove = set()
        for key in my_dict:
            if key.startswith("classifier"):
                keys_to_remove.add(key)
...
def vgg16_bn_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        my_dict = model_zoo.load_url(
            model_urls["vgg16_bn"], model_dir=PRETRAINED_MODELS_DIR
        )
        keys_to_remove = set()
        for key in my_dict:
            if key.startswith("classifier"):
...
def vgg19_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        my_dict = model_zoo.load_url(model_urls["vgg19"], model_dir=PRETRAINED_MODELS_DIR)
        keys_to_remove = set()
        for key in my_dict:
            if key.startswith("classifier"):
                keys_to_remove.add(key)
...
def vgg19_bn_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        my_dict = model_zoo.load_url(
            model_urls["vgg19_bn"], model_dir=PRETRAINED_MODELS_DIR
        )
        keys_to_remove = set()
        for key in my_dict:
            if key.startswith("classifier"):
