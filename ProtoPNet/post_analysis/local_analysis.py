...
from ProtoPNet.util.preprocess import mean, std, undo_preprocess_input_function
...
test_image_dir = args.imgdir[
    0
]  # './local_analysis/Painted_Bunting_Class15_0081/'
...
save_analysis_path = os.path.join(
    test_image_dir, model_base_architecture, experiment_run, load_model_name
)
...
from ProtoPNet.config.settings import dataset_config
...
preprocess = transforms.Compose(
    [transforms.Resize(img_shape), transforms.ToTensor(), normalize]
)
...
prototype_activations = ppnet_multi.module.distance_2_similarity(min_distances)
