...
import numpy as np
import ProtoPNet.find_nearest as find_nearest
import torch
...
from ProtoPNet.util.helpers import load_model_parallel, makedir
...
def main(args):
    ...
    img_size = ppnet_multi.module.img_shape

    train_dir = dataset_config["train_push_dir"]
    ...
    root_dir_for_saving_train_images = os.path.join(
        load_model_dir, load_model_name.split(".pth")[0] + "_nearest_train"
    )
    root_dir_for_saving_test_images = os.path.join(
        load_model_dir, load_model_name.split(".pth")[0] + "_nearest_test"
    )
    ...
    for j in range(ppnet_multi.module.num_prototypes):
        ...
        save_prototype_original_img_with_bbox(
            ...
            fname=os.path.join(
                root_dir_for_saving_train_images,
                str(j),
