...
from ProtoPNet.models.ProtoPNet._helpers import find_high_activation_crop
from ProtoPNet.models.ProtoPNet._helpers.receptive_field import compute_rf_prototype
...
def find_k_nearest_patches_to_prototypes(
    ...
    """
    logger.info("find nearest patches")
    start = time.time()
    n_prototypes = prototype_network_parallel.module.n_prototypes
    ...
    for search_batch_input, search_y in tqdm(dataloader):
    ...
    for j in range(n_prototypes):
        ...
        if full_save:
            ...
            for i, patch in enumerate(heaps[j]):
                ...
                logger.save_image(
                    image_name=dir_for_saving_images / f"nearest-{i + 1}_original.png",
                    image=patch.original_img,
                )

                # overlay (upsampled) activation on original image and save the result
                img_h = patch.original_img.shape[0]
                img_w = patch.original_img.shape[1]
                ...
                logger.save_image(
                    image_name=dir_for_saving_images
                    / f"nearest-{i + 1}_original_with_heatmap.png",
                    image=overlayed_original_img,
                )

                # if different from original image,
                # save the patch (i.e. receptive field)
                if patch.patch.shape[0] != img_h or patch.patch.shape[1] != img_w:
                    np.save(
    ...
    logger.info(f"\tfind nearest patches time: \t{end - start}")
